...
- 3-node Kubernetes v1.28 cluster on AWS EC2 (t3.medium instances, 2 vCPU + 4 GB each)
...
1. **Synthetic Sinusoidal**: Diurnal CPU pattern (baseline)
2. **Spike Workload**: Sudden 10x traffic spike at random intervals (stress test)

Traffic generated using `hey` (HTTP load generator) against the web service.
...
## VI. Results (To Be Populated After Experiments)

### A. Scaling Performance

| System | SLA Violations (%) | Avg Latency (ms) | Cold-Start Events |
|---|---|---|---|
| Default HPA | XX | XX | XX |
| Single-Metric LSTM | XX | XX | XX |
| **Our Full System** | **XX** | **XX** | **XX** |

### B. Cost Analysis

| System | Cost/Day ($) | Cost per SLA Violation Avoided ($) |
|---|---|---|
| Default HPA | XX | — |
| Our Full System | XX | XX |

### C. Prediction Accuracy

- Multi-Metric LSTM MAPE: **X%**
- Single-Metric LSTM MAPE: **Y%** (baseline)
- Improvement: **Z%**

### D. Drift Detection

- Number of drift events detected: **N**
- Average recovery time: **T seconds**

### E. Ablation Results

Shows each contribution adds measurable improvement:

| Configuration | SLA Violations | Cost/Day |
|---|---|---|
| E1 Full | XX | XX |
| E2 −Drift | +XX% | XX |
| E3 −Confidence | +XX% | XX |
| E4 −MultiMetric | +XX% | XX |
| E5 −Cost | same | XX |
...
1. **Synthetic workload**: Results may differ on real production traces. Future work will validate on Alibaba Cluster Trace.
2. **Single cluster size**: Tested on 3 nodes. Scalability to 100+ nodes not yet validated.
3. **Cost model**: Fixed $0.05/pod/hour; real cloud pricing varies by region and instance type.
4. **MAPE threshold (50%)**: Chosen empirically; adaptive thresholds are future work.

---

## VIII. Conclusion

We presented an integrated framework combining Multi-Metric LSTM, MAPE-based drift detection, cost-aware scaling, and confidence-gated self-healing for Kubernetes auto-scaling. To our knowledge, this is the first work to unify all four contributions with a complete CI/CD + GitOps pipeline. Experimental results demonstrate XX% reduction in SLA violations and XX% cost efficiency gain over reactive HPA. Future work includes: (i) validation on production workload traces (Alibaba, Google Borg); (ii) Transformer-based predictor replacing LSTM; (iii) multi-cluster federated scaling; and (iv) adaptive drift thresholds learned via reinforcement learning.
...
- [ ] Results with experiments ⚠ (run experiments, fill in XX values)
